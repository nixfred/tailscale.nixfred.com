...
const SURFACES = {
  editorial: scopeFor("[data-surface='editorial']"),
  crisis: scopeFor("[data-surface='crisis']"),
};
...
for (const surface of ['editorial', 'crisis']) {
  ...
  // Text on raised surfaces sitting on that ground.
  pair(surface, '--text', '--bg-alt', NORMAL_TEXT);
  pair(surface, '--text-muted', '--bg-alt', NORMAL_TEXT);
  pair(surface, '--text', '--card-bg', NORMAL_TEXT);
  pair(surface, '--text-muted', '--card-bg', NORMAL_TEXT);
  pair(surface, '--text-faint', '--card-bg', NORMAL_TEXT);
  pair(surface, '--accent', '--card-bg', NORMAL_TEXT);

  // Callout labels render as extra small caps TEXT on --bg-alt, so the
  // normal text threshold applies, not the indicator tier.
  for (const token of ['--callout-how', '--callout-gotcha', '--callout-wire', '--callout-field', '--callout-lab']) {
    pair(surface, token, '--bg-alt', NORMAL_TEXT);
  }

  // Diagram content inside module SVGs.
  pair(surface, '--diagram-text', '--diagram-bg', NORMAL_TEXT);
  pair(surface, '--diagram-accent', '--diagram-bg', NORMAL_TEXT);
  pair(surface, '--diagram-line', '--diagram-bg', UI_INDICATOR);

  // Alert text on its wash.
  pair(surface, '--alert-red', '--alert-red-bg', NORMAL_TEXT);

  // Focus ring is a state indicator, so 1.4.11 applies.
  pair(surface, '--accent-strong', '--bg', UI_INDICATOR);
}

// Advisory rows. Reported every run, never fail the run, so that a
// number nobody is asserting can never be mistaken for a pass.
// Decorative hairlines carry no 1.4.11 requirement.
pair('editorial', '--line', '--bg', ADVISORY);
pair('crisis', '--line', '--bg', ADVISORY);
pair('editorial', '--line', '--card-bg', ADVISORY);
